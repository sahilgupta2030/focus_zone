...
// Get single board details - All member
const getBoardById = asyncHandler(async (req, res) => {
    ...
    const workspaceId = board.workspace && board.workspace._id ? board.workspace._id : board.workspace;
    const workspace = await Workspace.findOne({
        _id: workspaceId,
        isDeleted: false
    }).populate("members.user");
...
// Get all board member - All member 
...
const removeMemberFromBoard = asyncHandler(async (req, res) => {
    ...
    const isOnBoard = board.members.some(
        (m) => String(m) === String(memberId)
    );
    if (!isOnBoard) throw new ApiError(400, "User is not a member of this board");

    // Optional guard: don't allow removing workspace owner from boards
